...
def test_the_abstract_fits_the_journal_word_limit():
    """Journal of Cheminformatics caps the abstract at 350 words.

    The submission form takes the abstract separately and requires it to match
    the manuscript, so an over-length abstract is caught at technical check
    rather than by a reader. Ours was 518 words when the limit was looked up.

    Counted on the generated plain-text version, which is what gets pasted
    into the form, not on the LaTeX source -- macros expand to numbers and
    markup does not count as words.
    """
    md = (ROOT / "paper" / "SUBMISSION.md").read_text()
    ...
    n = len(body.split())
    assert n <= 350, (
        f"the abstract is {n} words; Journal of Cheminformatics allows 350. "
        "Trim abstract.tex and re-run make_submission_abstract.py.")
    assert not re.search(r"\[\d+\]|\\cite", body), \
        "the abstract cites references; the journal does not allow it"
